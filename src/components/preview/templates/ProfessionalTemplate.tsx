...
export function ProfessionalTemplate({ resume }: TemplateProps) {
  ...
  return (
    ...
    >
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-4 text-xs" style={{ color: "#666" }}>
            ...
            {resume.personalInfo.linkedIn && (
              <span>
                LinkedIn: {resume.personalInfo.linkedIn.replace(/^https?:\/\/(www\.)?linkedin\.com\/(in\/)?/, '')}
              </span>
            )}
        ...
        {resume.personalInfo.summary && (
          <div className="mb-6">
            <h2
              ...
              style={{ color: "#2563eb", borderColor: "#2563eb", textTransform: "uppercase", letterSpacing: "0.5px" }}
            >
              Objective
            </h2>
        ...
        {resume.experience.length > 0 && (
          <div className="mb-6">
            ...
            {resume.experience.map((exp, idx) => (
              <div key={exp.id} className={idx < resume.experience.length - 1 ? "mb-4" : ""}>
                ...
                <p className="text-xs mb-2" style={{ color: "#2563eb", fontWeight: 600 }}>
                  {exp.position}
                </p>
                {exp.bulletPoints && exp.bulletPoints.length > 0 && (
                  <ul className="space-y-1">
                    {exp.bulletPoints.map((bullet, bIdx) => (
                      <li key={bIdx} className="text-xs flex items-start" style={{ color: "#444" }}>
      ...
      <div
        className="p-8 pl-6"
        style={{ width: "38%", backgroundColor: "#f8fafc", borderLeft: "3px solid #2563eb" }}
      >
        {/* Photo Placeholder */}
        ...
        {resume.personalInfo.summary && (
          <div className="mb-6">
            <h3
              ...
              style={{ color: "#1e40af", borderColor: "#cbd5e1", textTransform: "uppercase", letterSpacing: "0.5px" }}
            >
              Objective
            </h3>
        ...
        {technicalSkills.length > 0 && (
          <div className="mb-6">
            ...
            <div className="space-y-3">
              {/* Group by category */}
              {Object.entries({
                'Frontend': groupedSkills['frameworks']?.filter(s =>
                  ['react', 'vue', 'angular', 'next.js', 'tailwind'].some(tech =>
                    s.name.toLowerCase().includes(tech)
                  )
                ) || [],
                'Backend': groupedSkills['frameworks']?.filter(s =>
                  ['node', 'express', 'django', 'flask', 'spring'].some(tech =>
                    s.name.toLowerCase().includes(tech)
                  )
                ) || [],
          ...
          <div className="mb-6">
            ...
            {resume.hobbies.slice(0, 2).map((hobby) => (
              <div key={hobby.id} className="mb-2">
                ...
                {hobby.description && (
                  <p className="text-xs" style={{ color: "#64748b", lineHeight: "1.4" }}>
                    {hobby.description}
                  </p>
                )}
              </div>
